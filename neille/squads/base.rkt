...
(define (setup-card-browser)
  ...
  (define next-button 
    (send next-cards-view get-region))
  
  (define previous-button 
    (send previous-cards-view get-region))
  
  (define view-capacity 
    (send card-list-view get-capacity))
  ...
  (define max-pages 
    (ceiling (/ cards-length view-capacity)))
  
  (define page-number 1)
  
  (define page-content 
    (take cards view-capacity))
   ...
   (lambda ()
     (unless (>= page-number max-pages)
       ...
       (set! 
        page-content 
        (take-right 
         cards (* page-number view-capacity)))
       (send card-list from-list page-content)
  ...
  (set-region-callback!
   ...
   (lambda ()
     (unless (<= page-number 1)
       (send table- remove-cards (map get-card-view page-content))
       (set! page-number (- page-number 1))
       (set! page-content (take-right cards (* page-number view-capacity)))
       (send card-list from-list page-content))
     void)))
